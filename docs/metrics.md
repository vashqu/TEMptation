...
Present on every row of `axons.csv` and `image_summary.csv`, stamped once per run rather
than computed per-metric.
...
| `mode` | axon, image | `"normal"` if myelin pixel count ≥ `--myelin-threshold`, else `"pathological"` (with `--mode auto`, the default) | Which segmentation algorithm variant produced this row (normal computes g-ratio/myelin; pathological is axon-only) | **Not the same as `group`.** Auto-detected per image from mask content — two images in the same `group` can resolve to different `mode` if their myelin content differs. If this surprises you, either force `--mode normal`/`pathological` explicitly, or use the GUI's Segmentation tab, which has a clearly-labeled Auto-detect/Control/Pathology selector |
...
| `schema_version` | axon, image | `"2.0"` under the default `--mito-hole-handling fill`, `"1.0"` under `--mito-hole-handling legacy` | Which mitochondrial-area algorithm produced this row's numbers | `legacy` reproduces an older algorithm that only captured a thin rim of each mitochondrion — kept solely so old numbers can be reproduced exactly. **If a row's `schema_version` is `1.0`, treat every mitochondrial column in it as unreliable.** Not to be confused with `--schema {legacy,full}`, which controls column *layout*, not value correctness — see §9 |
...
| metric_name | level | formula | interpretation | notes |
|---|---|---|---|---|
| `axon_area_um2` | axon | pixel count × `pixel_size_um²` | Axoplasm cross-sectional area | Mito-inclusive under the default `--mito-hole-handling fill` (mitochondria inside the axon count as axoplasm) |
...
| `axon_vol_fraction` | axon | `axon_area_um2 / fiber_area_um2` | AVF — fraction of the fiber that is axoplasm | Identically `1.0` for every pathological axon, since myelin area is unmeasurable there (see `mito_per_myelin` and `image_demyelination_index` below for how this is handled) |
...
| `circularity` | axon | `4π·area / perimeter²` | Shape roundness | **Not `1.0` for a perfect circle.** `skimage.measure.perimeter` overestimates a digitized boundary's true length, so this caps around 0.90–0.98 even for a true disk, and is size-dependent (measured: 0.977 at a 10px radius, 0.905 at a 160px radius — same shape, different apparent value). Kept as-is for backward compatibility with existing QC thresholds calibrated against it. Prefer `axon_circularity_crofton` for any comparison across axons of different sizes |
| `axon_circularity_crofton` | axon | `4π·area / perimeter_crofton²` | Shape roundness, unbiased estimator | ≈1.00 across the size range where legacy `circularity` drifts from 0.98 to 0.90. Added alongside, not replacing, the legacy column |
...
## 3. Mitochondrial metrics — per-axon aggregates (`axons.csv`)

Computed identically regardless of `--mito-assignment` (the underlying mitochondrion
list differs by mode, not the aggregation formulas). `ddof=0` for std/CV here — see §4
for a parallel set of burden/shape metrics that use `ddof=1` instead.

| metric_name | level | formula | interpretation | notes |
|---|---|---|---|---|
| `mito_count` | axon | count of mitochondrial objects assigned to this axon | | `0` is a real value, not missing data |
| `mito_area_um2` | axon | sum of assigned mitochondrial pixel areas × `pixel_size_um²` | Total mitochondrial area in this axon | Same quantity as `mito_total_area_um2` (§4), computed via a separate code path — kept for backward compatibility |
...
## 4. Mitochondrial burden & shape (`axons.csv`)
...
| `mito_occupancy_ratio` | axon | `mito_total_area_um2 / axon_area_um2` | Fraction of axoplasm occupied by mitochondria | `axon_area_um2` is mito-inclusive (default `fill`), so this is a true areal fraction ∈ [0,1] |
...
| `mito_fragmentation_index` | axon | `mito_count / mito_total_area_um2` | Higher = same total mitochondrial area split into more, smaller pieces | `NaN` at `mito_count = 0` (never `inf`). Sensitive to very small/noise-scale mitochondria — see `docs/validation_report.md` §7 |
...
| `mito_per_myelin` | axon | `mito_total_area_um2 / myelin_area_um2` | Mitochondrial burden relative to myelin | `NaN` whenever `myelin_area_um2` is `NaN` — that includes every pathological axon by design; not something to "fix" by excluding those rows |
...
## 5. Mitochondrial spatial distribution (`axons.csv`)

| metric_name | level | formula | interpretation | notes |
|---|---|---|---|---|
| `mito_peripheralization_index` | axon | `mito_mean_dist_centroid_um / equivalent_axon_radius_um` | ~0 = mitochondria central, ~1 = near the axon boundary | Values above 1 can occur for irregular axons (equivalent radius is approximate) — worth a second look, not automatically wrong |
...
| `mito_clustering_index` | axon | `expected_nn_distance / observed_nn_distance` under a 2D Poisson (CSR) approximation | ~1 = random; >1 = clustered (observed NN smaller than expected); <1 = dispersed | `NaN` at `mito_count < 3` — the CSR approximation is unstable below that, even though `mito_mean_nn_distance_um` may already be available at count=2 |

---

## 6. Per-mitochondrion table (`mitochondria_metrics.csv`, `--write-mito-csv`)

One row per real mitochondrion — only produced under `--mito-assignment centroid` or
`overlap`. Under `--mito-assignment legacy` this table is empty, because that mode
crops and re-labels mitochondria independently per fiber, so a mitochondrion straddling
a watershed boundary between two axons is fragmented into pieces rather than staying a
single, real object — not something a "one row per mitochondrion" table can represent
meaningfully.
...
computed over every axon regardless of exclusion (see §7.6).
...
| `mito_outside_area_um2` | mitochondrial pixel area outside axoplasm+myelin | Segmentation-noise / unassigned mitochondria indicator | Structurally always the entire mitochondrial mask under `--mito-hole-handling legacy` — meaningful only under the default `fill` |
| `mito_outside_frac` | `mito_outside_area_um2 / total_mito_area` | Fraction of all mitochondrial pixels not attributed to any fiber | Same caveat as above — always `1.0` under `legacy` by construction |
...
### 7.2 Mitochondria-to-axon assignment diagnostics
...
### 7.4 MVF and demyelination index
...
| `image_demyelination_index` | `1 − (myelin_area_fraction_of_fov / reference)`, clipped to [0,1] | Demyelination relative to a reference (normal-tissue) myelin fraction | **`NaN` unless `--demyelination-reference` is explicitly supplied.** No default — the reference value is a biological calibration choice the tool cannot derive on its own. A reasonable choice is the median `myelin_area_fraction_of_fov` of a known-normal reference set; see `docs/validation_report.md` §4 for a worked example |

### 7.5 QC summary — see `docs/qc.md` for the flags themselves
...
| `image_noncanonical_mask_frac` | fraction of mask pixels outside `{0, myelin_val, mito_val, axoplasm_val}` | Mask quality indicator | A high value usually means a mask export/resaving artifact rather than intentional labeling — worth checking the mask's source pipeline |
...
## 9. Column layout vs. value correctness

Two different settings are easy to conflate:

- **`schema_version`** (a per-row CSV column, always present) — which mitochondrial-area
  algorithm actually produced that row's *numbers* (§1). `"2.0"` = the corrected
  algorithm (default), `"1.0"` = an older one kept only for reproducing old numbers.
- **`--schema {legacy,full}`** (a CLI flag) — which *columns* get written at all.
  `legacy` writes only the original pre-refactor column set, in its original order, for
  downstream scripts that don't expect new columns. `full` (default) writes every
  column this version of the tool computes.

You can have `--schema legacy` with `schema_version="2.0"` (old columns, corrected
values) or `--schema full` with `schema_version="1.0"` (all columns, but with the older
mitochondrial-area algorithm active on purpose, e.g. for regression testing). **Check
`schema_version` — not `--schema` — when deciding whether a mitochondrial number is
trustworthy.**

---

## 10. QC flags and exclusion columns
