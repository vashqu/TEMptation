...
----------------------------
Retired pre-Phase-7 Tkinter GUI, kept importable during the gui/
rebuild's transition window (IMPLEMENTATION_BLUEPRINT.md Phase 7 risk
note: "keep the old GUI importable ... until the new one is
accepted"). `measure_nerve_gui.py` now launches the new step-rail app
in gui/app.py; run this module directly only if you need the old
single-image/batch-folder workflow it doesn't yet fully replace
(multi-group runs, QC, the visual review panel).
...
Usage:
    python measure_nerve_gui_legacy.py
"""
